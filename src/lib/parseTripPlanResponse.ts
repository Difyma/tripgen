/**
 * Парсинг и нормализация ответа модели: извлечение JSON и приведение к TripPlanResponse.
 * Этапы: extractJsonFromText → safeParseJson → normalizeTripPlanResponse.
...
/** Извлекает строку с JSON из сырого ответа (чистый JSON, ```json...``` или текст с {...}). */
...
/** Парсит JSON-строку; при ошибке возвращает null. */
...
/** Массив строк: из массива — строки + разбиение элементов по \n; из одной строки — разбиение по \n. */
...
/**
 * Нормализует сырой объект после parse в типизированный TripPlanResponse.
 * Учитывает: itinerary как объект, morning/daytime/evening как строка, stars как number/string/null,
 * пустой bookingUrl, отсутствующие recommendedAreas и т.п.
 */
...
/**
 * Полный пайплайн: извлечь JSON → распарсить → нормализовать.
